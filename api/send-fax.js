...
// ======================================================
// CloudConvert: xlsx → PDF 변환 (wait 방식으로 최적화)
// ======================================================
async function convertXlsxToPdf(xlsxBase64, filename) {
  // Sync job: wait 태스크로 즉시 응답 대기 (polling 오버헤드 감소)
  ...
  if (!jobRes.ok) throw new Error("CloudConvert job 실패: " + jobText);
  const jobData = JSON.parse(jobText).data;

  if (jobData.status !== "finished") {
    const errs = (jobData.tasks || []).filter(t => t.status === "error").map(t => t.message).join("; ");
    throw new Error("CloudConvert 변환 실패: " + (errs || jobData.status));
  }
